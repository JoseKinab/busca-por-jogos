...
for (let i = 0; i < buttons.length; i++) {
  buttons[i].addEventListener("click", function(e) {
    // Verifica se o coração sólido está presente no conteúdo HTML
    const isSolidHeart = buttons[i].innerHTML.includes('fa-solid fa-heart');

    // Atualiza o conteúdo com base na verificação anterior
    if (isSolidHeart) {
      buttons[i].innerHTML = '<i class="fa-regular fa-heart"></i>';
    } else {
      buttons[i].innerHTML = '<i class="fa-solid fa-heart" style="color: #d33c69;"></i>';
    }
  });
}
